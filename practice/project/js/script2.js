...
document.addEventListener('DOMContentLoaded', () => {
  ...
  addForm.addEventListener('submit', function (event) {
    ...
    if (newFilm) {
      ...
      if (newFilm.length > 21) {
        newFilm = `${newFilm.substring(0, 24)}...`;
      }

      if (favorite) {
        console.log('Добавляем любимый фильм');
      } else {
        console.log('Любимый фильм не добавлен');
      }
  ...
  function createMoviesList(films, parent) {
    ...
    parent.innerHTML = ''; // ! Если в innerHTML оставить пустую строку, то всё её содержимое просто удалится
